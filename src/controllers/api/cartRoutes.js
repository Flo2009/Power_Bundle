...
exports.createCart = async (req, res) => {
  try {
    // Check if the customer is authenticated using req.isAuthenticated()
    if (!req.isAuthenticated()) {
      return res.status(401).json({ error: "customer not authenticated" });
    }

    // Log customer data from the session
    console.log("customer in session when creating cart:", req.customer);

    // customer is authenticated, proceed to create the cart
